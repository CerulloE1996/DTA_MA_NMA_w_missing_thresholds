...
#xu
plot(col = "green", xlim = c(-5, 5), density(rnorm(10000,0, 1)))
lines(col = "red", density(rnorm(10000,2, 1)))
lines(col = "green", density(runif(10000, min = -1.5, max = -1.40)))
lines(col = "green", density(runif(10000, min = 0.1, max = 0.2)))
lines(col = "red", density(runif(10000, min = 0.5, max = 0.6)))
lines(col = "red", density(runif(10000, min = 2.5, max = 2.6)))


#xu
plot(col = "green", xlim = c(-5, 5), density(rnorm(10000,0, 0.6)))
lines(col = "red", density(rnorm(10000,2, 1.3)))
lines(col = "black", density(runif(10000, min = -0.1, max = 0)))
lines(col = "black", density(runif(10000, min = 2.1, max = 2.2)))
...
### imperfect GS, CD
#file <- file.path(file = "mvp_wells_CD_random_cutpoints_logit_2.stan") 


# SCALE
### perfect GS, CI
file <- file.path(file = "mvp_wells_CI_random_cutpoints_perfectGS_logit_scale.stan") 

### perfect GS, CD
#file <- file.path(file = "mvp_wells_CD_random_cutpoints_perfectGS_logit_scale.stan") 

### imperfect GS, CI
#file <- file.path(file = "mvp_wells_CI_random_cutpoints_logit_scale.stan") 

### imperfect GS, CD
#file <- file.path(file = "mvp_wells_CD_random_cutpoints_logit_scale.stan") 
...
s<- summary(subset_draws(as_draws(meta_model2r),chain=c(2,3,4),
...
m2 <- readRDS(file = "Wells_CD_perfectGS_xu_2000_norm10.rds")   ###  M2 - perfect GS, CD
m3 <- readRDS(file = "Wells_CI_xu_1000_norm10.rds")   ###  M3 - IGS, CI
m4 <- readRDS(file = "Wells_CD_xu_2000_norm10.rds")   ###  M4 - IGS, CD
...
cat_nd_plot <- ggplot(data_nd, aes(x=location, y = m, ymin= l,ymax= u, shape = Model)) + 
  ...
  scale_y_continuous(breaks = seq(0, 1, 0.10), limits = c(0, 1)) + 
  scale_x_discrete( labels = rep("  ", length(data_Se$location))) + 
  geom_text(aes( label = paste0( m*100," ", "[",l*100,",", u*100 , "]") ,hjust = 1.25, vjust = -0.25), alpha = 0.35, size = 4.5) +
...
tiff("wells_figure_summary_xu.tif",units = "in", width = 9, height=7, res=500, compression = "lzw")
se_plot + sp_plot
dev.off()

tiff("wells_figure_cats_xu.tif",units = "in", width = 9, height=5, res=500, compression = "lzw")
cat_d_plot + cat_nd_plot
dev.off()
...
tiff("wells_sroc_scale.tif",units = "in", width = 7, height=5, res=500, compression = "lzw")
g
dev.off()
