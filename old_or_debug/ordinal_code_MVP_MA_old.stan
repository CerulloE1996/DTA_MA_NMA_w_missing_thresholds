functions {
          ...
          matrix convex_combine_cholesky( matrix global_chol_cor, 
                                          matrix local_chol_cor, 
                                          real alpha) {
...
real induced_dirichlet_lpdf( vector C, 
        ...
        int n_cat = num_elements(C) + 1;
        vector[n_cat - 1] anchoredcutoffs = C - anchor;
    
        vector[n_cat] p_cumul;
        vector[n_cat] p_ord;
        matrix[n_cat, n_cat] J = rep_matrix(0, n_cat, n_cat);
    
        p_cumul[1:(n_cat - 1)] = Phi(anchoredcutoffs); 
        p_cumul[n_cat] = 1;
       
        p_ord[1] =  p_cumul[1];
        for (k in 2:(n_cat - 1))
          p_ord[k] = p_cumul[k] - p_cumul[k - 1];
        p_ord[n_cat] = 1 - p_cumul[n_cat - 1];
        
        // Baseline column of Jacobian
        for (k in 1:n_cat) J[k, 1] = 1;
...

      {
        for (s in 1:n_studies) {
              ...
              // GHK algorithm for generating TruncMVN. See: https://github.com/stan-dev/example-models/blob/master/misc/multivariate-probit/probit-multi-good.stan#L11
                                ...
                                for (i in 1:nt) { // loop over each test - make sure binary tests are at the start 
                                 ...
                                 // Binary Likelihoods (tests w/ 1 threshold)
                                  if (Thr[i] == 1) { 
                                            ...
                                            bound_d_bin[1:num_binary_tests]  = Phi(  -(  to_vector(nu[s,1,1:num_binary_tests])  + prev_d ) / L_Omega_d[s,i,i] ); 
                                            bound_nd_bin[1:num_binary_tests] = Phi(  -(  to_vector(nu[s,2,1:num_binary_tests]) + prev_nd )  / L_Omega_nd[s,i,i] );
                                              if (y[n,i,s] == 1) {
                                                     z_d[i]   = inv_Phi(bound_d_bin[i] + (1 - bound_d_bin[i])*u_d1);      
                                                     z_nd[i]  = inv_Phi(bound_nd_bin[i] + (1 - bound_nd_bin[i])*u_nd1);    
                                                     y1d[i]   = log1m(bound_d_bin[i]);  // Jacobian adjustment
                                                     y1nd[i]  = log1m(bound_nd_bin[i]); // Jacobian adjustment
                                              } else if (y == 0) {  // y == 0
                                                     z_d[i]   = inv_Phi(bound_d_bin[i]*u_d1);
                                                     z_nd[i]  = inv_Phi(bound_nd_bin[i]*u_nd1);
                                                     y1d[i]   = log(bound_d_bin[i]);  // Jacobian adjustment
                                                     y1nd[i]  = log(bound_nd_bin[i]); // Jacobian adjustment
                                              } else if (y == 999) { 
                                                     z_d[i] =   L_Omega_d[c, i, i]  * inv_Phi(u_d1);
                                                     z_nd[i] =  L_Omega_nd[c, i, i] * inv_Phi(u_nd1);
                                                     y1d[i]  = 0;
                                                     y1nd[i] = 0;
                                              }
                                          ...
                                          bound_d[1:Thr[3]]  =  Phi( ( C_d[s,1:Thr[3]]   - ( nu[s,1,3]  + prev_d  ) ) /   L_Omega_d[s,i,i]  ); // diseased
                                          bound_nd[1:Thr[3]] =  Phi( ( C_nd[s,1:Thr[3]]  - ( nu[s,2,3]  + prev_nd ) ) /   L_Omega_nd[s,i,i]   ); // non-diseased
                                           int K =   (Thr[i] + 1) ; 
                                              if (y[n,i,s] == K) {
                                                z_d[i]   = inv_Phi(bound_d[K-1] + (1 - bound_d[K-1])*u_d1);      
                                                z_nd[i]  = inv_Phi(bound_nd[K-1] + (1 - bound_nd[K-1])*u_nd1);    
                                                y1d[i]   = log1m(bound_d[K-1]);  // Jacobian adjustment
                                                y1nd[i]  = log1m(bound_nd[K-1]); // Jacobian adjustment
                                              }
                                             for (J in 2:(K-1)) {
                                              if (y[n,i,s] == J) {
                                                z_d[i]   = inv_Phi(bound_d[J-1] + (bound_d[J] - bound_d[J-1])*u_d1);      
                                                z_nd[i]  = inv_Phi(bound_nd[J-1] + (bound_nd[J] - bound_nd[J-1])*u_nd1);    
                                                y1d[i]   = log(  (bound_d[J] - bound_d[J-1]) );  // Jacobian adjustment
                                                y1nd[i]  = log( (bound_nd[J] - bound_nd[J-1]) );  // Jacobian adjustment
                                              }
                                             }
                                              if (y[n,i,s] == 1) {
                                              z_d[i]   = inv_Phi(bound_d[1]* u_d1);
                                              z_nd[i]  = inv_Phi(bound_nd[1]* u_nd1);
                  ...
                  } // end of "for (n in 1:ns[s])" loop. 
          
        } // end of "for (s in 1:n_studies)" loop
...
model {
          ...
          for (s in 1:n_studies) {
                C_d[s,]   ~  induced_dirichlet(alpha_d, 0.0);
                C_nd[s,]  ~  induced_dirichlet(alpha_nd, 0.0);
          }

          to_vector(sd1) ~ normal(0, 0.50); 

          for (t in 1:nt) 
              L_Omega_bs[t, ,] ~ lkj_corr_cholesky(2.0);
          
          mu[2,1] ~ normal(-1.70, 0.40); 
          mu[1,1] ~ normal(0.75, 0.40); 
          mu[, 2] ~ std_normal();  
          mu[, 3] ~ std_normal();

          beta_d  ~ beta(2.0, 2.0);
          beta_nd ~ beta(2.0, 2.0);

          L_Omega_global_d  ~ lkj_corr_cholesky(4.0);
          L_Omega_global_nd ~ lkj_corr_cholesky(4.0);

          for (s in 1:n_studies) {
             R_diff_L_d[s,]  ~ lkj_corr_cholesky(4.0);
             R_diff_L_nd[s,] ~ lkj_corr_cholesky(4.0);
          }
...
generated quantities {
    ...
    for (i in 1:(Thr[3]+1)) 
       ...
       C_dm[1] =    inv_Phi(p_dm[1]); 
    for (i in 2:Thr[3]) 
      C_dm[i] =    inv_Phi(p_dm[i] + Phi(C_dm[i-1])); 

       C_dm2[1] =   inv_Phi(p_dm_sim[1,1]); 
    for (i in 2:Thr[3]) 
       C_dm2[i] =    inv_Phi(p_dm_sim[1,i] + Phi(C_dm2[i-1])); 

       C_ndm[1] =    inv_Phi(p_ndm[1]); 
    for (i in 2:Thr[3]) 
      C_ndm[i] =    inv_Phi(p_ndm[i] + Phi(C_ndm[i-1])); 

       C_ndm2[1] =   inv_Phi(p_ndm_sim[1,1]); 
    for (i in 2:Thr[3]) 
       C_ndm2[i] =    inv_Phi(p_ndm_sim[1,i] + Phi(C_ndm2[i-1])); 
      ...
      L[1]  =        Phi( C_dm[1] -  mu[1, 3]) ; 
      M[1]  =        Phi( C_dm[2] -  mu[1, 3]) - Phi( C_dm[1] -  mu[1, 3]); 
      H[1]  =    1 - Phi( C_dm[2] -  mu[1, 3]) ; 
  
      L[2]  =        Phi( C_ndm[1] -  mu[2, 3]) ; 
      M[2]  =        Phi( C_ndm[2] -  mu[2, 3]) - Phi( C_ndm[1] -  mu[2, 3]) ; 
      H[2]  =    1 - Phi( C_ndm[2] -  mu[2, 3]) ; 

      // Wells score - dichotomise as low vs (moderate + high) [first cut-off]
      Se[3] = 1 - L[1]; // M[1] + H[1]; =   Phi( mu[1,3] -  C_dm[1]    );
      Sp[3] =     L[2] ; // 1- (M[2] + H[2]); = 1 - Phi(  mu[2,3] - C_dm[1]  );

  for (t in 1:nt)
    pred[1:2, t] = multi_normal_cholesky_rng(to_vector(mu[1:2, t]), diag_pre_multiply(sd1[t, 1:2], L_Omega_bs[t, ] ));

      // Global Estimates
    Se_pred[1]  =        Phi(   pred[1,1]  );
    Sp_pred[1]  =    1 - Phi(   pred[2,1]  );
    Se_pred[2]  =        Phi(   pred[1,2]  );
    Sp_pred[2]  =    1 - Phi(   pred[2,2]  );

      // Wells score - prob. of each category
    L_pred[1]  =        Phi( C_dm2[1] -  pred[1, 3]) ; 
    M_pred[1]  =        Phi( C_dm2[2] -  pred[1, 3]) - Phi( C_dm2[1] -  pred[1, 3]); 
    H_pred[1]  =    1 - Phi( C_dm2[2] -  pred[1, 3]) ; 

    L_pred[2]  =        Phi( C_ndm2[1] -  pred[2, 3]) ; 
    M_pred[2]  =        Phi( C_ndm2[2] -  pred[2, 3]) - Phi( C_ndm2[1] -  pred[2, 3])  ; 
    H_pred[2]  =    1 - Phi( C_ndm2[2] -  pred[2, 3]) ; 
...

 {
  ...
  }
    {
      ...
      for (s in 1:n_studies)  {
        ...
        se[s,1]   =         Phi(  nu[s,1,1] );
        sp[s,1]   =    1 -  Phi(  nu[s,2,1] );
        se[s,2]   =         Phi(  nu[s,1,2] );
        sp[s,2]   =    1 -  Phi(  nu[s,2,2] );

      // Wells score - prob. of each category
    l[s,1]  =        Phi( C_d[s,1] -  nu[s,1,3] ) ; 
    m[s,1]  =        Phi( C_d[s,2] -  nu[s,1,3] )    - Phi( C_d[s,1] -  nu[s,1,3] ) ; 
    h[s,1]  =    1 - Phi( C_d[s,2] -  nu[s,1,3] ) ; 

    l[s,2]  =        Phi( C_nd[s,1] -  nu[s,2,3] ) ; 
    m[s,2]  =        Phi( C_nd[s,2] -  nu[s,2,3] )  - Phi( C_nd[s,1] -  nu[s,2,3] ) ; 
    h[s,2]  =    1 - Phi( C_nd[s,2] -  nu[s,2,3] ) ;
