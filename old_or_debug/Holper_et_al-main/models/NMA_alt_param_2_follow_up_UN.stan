...
transformed parameters {
        ...
        vector[2] nu[n_studies];  
          ...
          //// nu[s,] ~ normal(0, sigma).
          //// captures between-study var (aka the "study effect") which is shared across tests, even if using UN (i.e. test-specific) structure 
          //// - hence NMA as using indirect evidence (borrowing info on other tests from other studies)
          for (s in 1:n_studies) {
             nu[s, ] =   diag_pre_multiply(sigma, L_Omega) * to_vector(z[s, ]);
          }
