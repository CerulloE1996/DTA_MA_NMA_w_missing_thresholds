...
transformed parameters {
      ...
      for (s in 1:n_studies) 
             eta[s, ] =   diag_pre_multiply(sigma, L_Omega) * to_vector(z[s, ])    ;   // eta[s,] ~ normal(0, Sigma) 
          ...
          for (n in 1:N) { // likelihood is only implemented for the observed data points (hence loop through the N observations only)
                      for (d in 1:2) 
                        ...
                        logit_pi[n, 1] = intercept[Test_Threshold[n],1] +   fu_coeff[Test_Threshold[n], 1]*fu[n] +  eta[Study[n], 1] + delta[n, 1] ;
                        logit_pi[n, 2] = intercept[Test_Threshold[n],2] +   fu_coeff[Test_Threshold[n], 2]*fu[n] +  eta[Study[n], 2] + delta[n, 2] ;
