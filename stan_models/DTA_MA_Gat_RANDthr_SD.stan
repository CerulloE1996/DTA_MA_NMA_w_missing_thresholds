...
model {
        ...
        {
            for (s in 1:n_studies) {
              target += induced_dirichlet_v2_lpdf(to_vector(Ind_Dir_ord_prob[s, 1:n_cat]) | to_vector(Ind_Dir_cumul_prob[s, 1:n_thr]), alpha[1:n_cat]);
            }
            if (prior_only == 0) {
                  for (k in 1:n_cat) {
                      target += log_kappa;
...
generated quantities {
          ...
          //// Calculate predictive accuracy:
          {
                ...
                for (k in 1:n_thr) {
                      Fp_pred[k] =   1.0 - Phi(C_MU[k] - bs_pred[1]);
                      Sp_pred[k] =   1.0 - Fp_pred[k];
                      Se_pred[k] =   1.0 - Phi(C_MU[k] - bs_pred[2]);
                }
          }
