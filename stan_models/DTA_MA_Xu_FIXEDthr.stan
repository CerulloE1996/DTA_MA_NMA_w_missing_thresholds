...
transformed parameters { 
        ...
        ////
        matrix[n_sets_of_cutpoints, n_thr] Ind_Dir_anchor;
        matrix[n_sets_of_cutpoints, n_thr] Ind_Dir_cumul;
        matrix[n_sets_of_cutpoints, n_thr] Ind_Dir_cumul_prob;
        matrix[n_sets_of_cutpoints, n_cat] Ind_Dir_ord_prob;
        ////
        matrix[n_sets_of_cutpoints, n_thr] C;
        ...
        for (c in 1:n_sets_of_cutpoints) {
                   ...
                   Ind_Dir_cumul[c, 1:n_thr] =  (C[c, ] - Ind_Dir_anchor[c, 1:n_thr]);
                   Ind_Dir_cumul_prob[c, 1:n_thr] = Phi(Ind_Dir_cumul[c, 1:n_thr]);
                   //// Ind-Dir ordinal probs:
                   Ind_Dir_ord_prob[c, 1] = Ind_Dir_cumul_prob[c, 1] - 0.0;
                   for (k in 2:n_thr) {
                       Ind_Dir_ord_prob[c, k] = Ind_Dir_cumul_prob[c, k] - Ind_Dir_cumul_prob[c, k - 1]; // since probs are INCREASING with k
                   }
                   Ind_Dir_ord_prob[c, n_cat] =  1.0 - Ind_Dir_cumul_prob[c, n_cat - 1];
        }
        //// Between-study model for log_scale parameters:
        log_scale_mu[1] = 0.0;
        log_scale_SD[1] = 0.0;
...
generated quantities {
          ...
          for (k in 1:n_thr) {
                Fp[k] =   1.0 - Phi((beta_mu[1] - C[1, k])/scale_mu[1]);
                Sp[k] =   1.0 - Fp[k];
                Se[k] =   1.0 - Phi((beta_mu[2] - C[2, k])/scale_mu[2]);
          }
          //// Calculate predictive accuracy:
          {
                ...
                for (k in 1:n_thr) {
                      Fp_pred[k] =   1.0 - Phi((beta_pred[1] - C[1, k])/scale_pred[1]);
                      Sp_pred[k] =   1.0 - Fp_pred[k];
                      Se_pred[k] =   1.0 - Phi((beta_pred[2] - C[2, k])/scale_pred[2]);
                }
          }
