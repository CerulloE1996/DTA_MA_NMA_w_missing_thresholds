...
transformed parameters { 
        ...
        // Subsequent cutpoints:
        {
              ...
              ////
              trans_C[, 2:n_thr]  = exp(unc_C_normal[, 2:n_thr]);
              //// deriv of trans_C w.r.t unc_C_normal:
              Jacobian_for_unc_C_to_trans_C += sum(unc_C_normal[, 2:n_thr]);
              ...
              // }
              // trans_C[, 2:n_thr]  = exp(unc_C_normal[, 2:n_thr] + 0.5 * unc_C_normal_SD_sq_mat[, 2:n_thr]);
              // //// deriv of trans_C w.r.t unc_C_normal:
        ...
        ////
        for (k in 2:n_thr) {
            real stuff_to_exp = unc_C_normal_MU[k]  + 0.5 * unc_C_normal_SD_sq[k];
            trans_C_MU[k]  = exp(unc_C_normal_MU[k] + 0.5 * unc_C_normal_SD_sq[k]);
            ////real stuff_to_exp = unc_C_normal_MU[k];
            trans_C_MED[k]    = exp(unc_C_normal_MU[k]);
            //// deriv of trans_C_MU w.r.t unc_C_normal_MU:
            //// deriv of trans_C_MU[k]  = exp(unc_C_normal_MU[k] + 0.5 * unc_C_normal_SD_sq[k]) w.r.t  unc_C_normal_MU is:
            //// exp(stuff_to_exp) * deriv_of_correction_wrt_unc_C_normal_MU = exp(stuff_to_exp) * 1;
            Jacobian_for_unc_C_to_trans_C_MU += stuff_to_exp;  //// deriv of trans_C_MU w.r.t unc_C_normal_MU = log(abs(trans_C_MU[k])) + log(abs(1));
        ...
        for (k in 2:n_thr) {
                C_MED[1, k] = C_MED[1, k - 1] + trans_C_MED[k]; // No Jacobian needed for transformation trans_C_MED -> C_MED
          ...
          //// Summary cutpoints:
          {
            ...
            for (i in 1:n_sims) {
                     ...
                     for (k in 2:n_thr) {
                            ...
                            trans_C_sim[i, k] = exp(unc_C_normal_sim + 0.50 * square(unc_C_normal_SD[k]));
