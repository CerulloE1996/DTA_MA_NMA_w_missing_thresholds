...
parameters {
        ...
        array[n_sets_of_cutpoints] simplex[n_cat] phi;
        ////
        //// vector<lower=exp(log_alpha_lb), upper=exp(log_alpha_ub)>[2] kappa;  
        vector<lower=kappa_lb>[n_sets_of_cutpoints] kappa;  
...
transformed parameters { 
        ...
        ////
        array[n_sets_of_cutpoints] matrix[n_studies, n_thr] Ind_Dir_anchor;
        ...
        vector[n_sets_of_cutpoints] anchors = rep_vector(0.0, n_sets_of_cutpoints);
        ////
        array[n_sets_of_cutpoints] matrix[n_studies, n_thr] C;
        array[n_sets_of_cutpoints] vector[n_cat] alpha;
        vector[n_sets_of_cutpoints] log_kappa = log(kappa);
        ////
        //// Compute alpha:
        ////
        for (c in 1:n_sets_of_cutpoints) {
            for (k in 1:n_cat) {
                 alpha[c][k] = exp(log_kappa[c] + log(phi[c][k]));
            }
        }
        ////
        //// Initialise matrices:
        ...
        for (c in 1:n_sets_of_cutpoints) {
                 ...
                 Ind_Dir_anchor[c]     = rep_matrix(anchors[c], n_studies, n_thr);
        }
        ////
        ...
        ////
        for (c in 1:n_sets_of_cutpoints) {
                Ind_Dir_cumul_prob[c] = Phi(C[1] - Ind_Dir_anchor[c]);
                for (s in 1:n_studies) {
...
model {
        ...
        for (c in 1:n_sets_of_cutpoints) {
            target += normal_lpdf(kappa[c] | prior_kappa_mean[c], prior_kappa_SD[c]);
