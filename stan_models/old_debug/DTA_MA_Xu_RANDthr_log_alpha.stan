...
parameters {
        ...
        matrix<lower=log_alpha_lb, upper=log_alpha_ub>[2, n_cat] alpha_raw; // Forces the ind_Dirichlet "alpha" to be >= 1
...
transformed parameters { 
        ...
        array[2] matrix[n_studies, n_cat] ord_prob;   // Ordinal probs for the likelihood
        ...
        matrix[2, n_studies] log_lik_ord;     // log_lik storage
        ////
        ...
        matrix[2, n_cat] log_alpha = exp(alpha_raw);
        ...
        for (s in 1:n_studies) {
                  for (c in 1:2) {
                      for (cut_i in 1:to_int(n_cutpoints[c, s])) {
                             ...
                             logit_cumul_prob[c][s, cut_i] = (beta[c, s] - C[c][s, k]);
                      }
                  }
        }
...
model {
        ...
        for (c in 1:2) {
            target += normal_lpdf(alpha_raw[c, ] | prior_kappa_mean[c], prior_kappa_SD[c]); // putting prior DIRECTLY on parameter so NO JACOBIAN needed here !!
        }
        for (c in 1:2) {
          for (s in 1:n_studies) {
        ...
        target += sum(alpha_raw);  // Jacobian adjustment for the transformation
        ////
        //// Likelihood / Model:
        {
...
generated quantities {
          ...
          for (k in 1:n_thr) {
                Fp[k] =   inv_logit((beta_mu[1] - C_mu[1, k]));
                Sp[k] =   1.0 - Fp[k];
                Se[k] =   inv_logit((beta_mu[2] - C_mu[2, k]));
          }
          //// Calculate predictive accuracy:
          {
                vector[2] beta_pred =  to_vector(multi_normal_cholesky_rng(beta_mu, beta_L_Sigma));
                for (k in 1:n_thr) {
                      Fp_pred[k] =   inv_logit((beta_pred[1] - C_mu[1, k]));
                      Sp_pred[k] =   1.0 - Fp_pred[k];
                      Se_pred[k] =   inv_logit((beta_pred[2] - C_mu[2, k]));
                }
          }
