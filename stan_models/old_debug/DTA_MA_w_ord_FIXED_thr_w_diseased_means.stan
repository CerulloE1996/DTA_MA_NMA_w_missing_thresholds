...
parameters {
        ...
        real<lower=0> kappa;  
        simplex[n_cat] phi;  // for induced-dirichlet between-study cutpoint model
...
transformed parameters { 
        ...
        // real scale_d_mu = exp(log_scale_d_mu);
        // vector[n_studies] scale_d; //  = exp(log_scale_d);
        vector[n_studies] scale_d = rep_vector(1.0, n_studies); //// Fix scale to 1.0
        ...
        //// for induced-dirichlet between-study cutpoint model:
        vector<lower=0>[n_thr + 1] alpha =  phi*kappa;
        ...
        for (s in 1:n_studies) {
                ...
                if (x_non_diseased[s, 1] != 999) { //// If non-missing (i.e. only if study reports @ this ordinal cutpoint!)
                         cumul_cond_prob_nd[s, 1] = Phi_approx(beta_nd[s] - cutpoints[s, 1]);
                }
                
                for (k in 2:n_thr) {
                     if (x_non_diseased[s, k] != 999) { //// If non-missing (i.e. only if study reports @ this ordinal cutpoint!)
                         cumul_cond_prob_nd[s, k] = Phi_approx(beta_nd[s] - cutpoints[s, k]) / Phi_approx(beta_nd[s] - cutpoints[s, k - 1]);
                     }
                }
                
                //// Diseased group (D+):
                if (x_diseased[s, 1] != 999) { //// If non-missing (i.e. only if study reports @ this ordinal cutpoint!)
                         cumul_cond_prob_d[s, 1] = Phi_approx((beta_d[s] - cutpoints[s, 1])/scale_d[s]);
                }
                
                for (k in 2:n_thr) {
                   if (x_diseased[s, k] != 999) { //// If non-missing (i.e. only if study reports @ this ordinal cutpoint!)
                         cumul_cond_prob_d[s, k] = Phi_approx((beta_d[s] - cutpoints[s, k])/scale_d[s]) /  Phi_approx((beta_d[s] - cutpoints[s, k - 1])/scale_d[s]);
                   }
                }
...
model {
        ...
        target += normal_lpdf(kappa  | 0.0, 10.0);
        ...
        for (s in 1:n_studies)  {
                   target += induced_dirichlet_lpdf(cutpoints[s, 1:n_thr] | alpha, 0.0);
        }
...
generated quantities {
          ...
          vector[n_thr] cutpoints_mu;
          vector[n_cat] prob_ord_mu;
          
          {
          
                 int n_sims = 10;
                 array[n_sims] vector[n_cat] prob_ord_mu_sim;
                  
                 for (i in 1:n_sims) {
                     prob_ord_mu_sim[i,]  =  dirichlet_rng(alpha);
                 }
                 ...
                 cutpoints_mu[1] =        inv_Phi(prob_ord_mu[1]); 
                 for (k in 2:n_thr) {
                       cutpoints_mu[k] =        inv_Phi( prob_ord_mu[k] + Phi_approx(cutpoints_mu[k - 1]) ); 
                 }
          ...
          for (s in 1:n_studies) {
                for (k in 1:n_thr) {
                    se[s, k] =       Phi_approx((cutpoints[s, k] - beta_d[s])/scale_d[s]);
                    sp[s, k] = 1.0 - Phi_approx(cutpoints[s, k]  - beta_nd[s]);
                }
          }
          ...
          for (k in 1:n_thr) {
              real scale_d_mu = 1.0;
              Se[k] =        Phi_approx((cutpoints_mu[k] - beta_d_mu)/scale_d_mu);
              Sp[k] =  1.0 - Phi_approx(cutpoints_mu[k]  - beta_nd_mu);
          }
          ...
          //// Calculate predictive accuracy:
          {
              ...
              for (k in 1:n_thr) {
                  Se_pred[k] =        Phi_approx((cutpoints_mu[k] - beta_d_pred)/scale_d_pred);
                  Sp_pred[k] =  1.0 - Phi_approx(cutpoints_mu[k]  - beta_nd_pred);
              }
          }
