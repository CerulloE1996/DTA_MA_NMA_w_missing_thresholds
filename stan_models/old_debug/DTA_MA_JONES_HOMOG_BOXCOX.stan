...
data {
      ...
      real<lower=0.0> prior_boxcox_lambda_SD;
...
parameters {
      ...
      ////
      real<lower=-5.0, upper = 5.0> lambda;   // box-cox param
...
transformed parameters { 
        ...
        if (use_box_cox == 0) {
              ...
              C[1, ]  = to_row_vector(box_cox(cts_thr_values_nd, lambda));
              C[2, ]  = to_row_vector(box_cox(cts_thr_values_d,  lambda));
        }
...
generated quantities {
          ...
          for (k in 1:n_thr) {
                Fp[k] =   inv_logit((beta_mu[1] - C[1, k])/exp(log_scale_mu[1]));
                Sp[k] =   1.0 - Fp[k];
                Se[k] =   inv_logit((beta_mu[2] - C[2, k])/exp(log_scale_mu[2]));
          }
