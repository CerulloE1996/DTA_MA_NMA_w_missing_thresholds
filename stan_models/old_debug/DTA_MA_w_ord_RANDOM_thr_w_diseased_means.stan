...
parameters {
      ...
      array[n_studies] ordered[n_thr] C_non_diseased;  // Global cutpoints 
      array[n_studies] ordered[n_thr] C_diseased;  // Global cutpoints
...
transformed parameters { 
      ...
      //// Estimate means in the non-diseased group (but consider fixing the scales in this group for identifiability -
      //// especially if you also want cutpoints to vary between studies):
...
model {
      ...
      for (s in 1:n_studies) {
              ...
              if (x_non_diseased[s, 1] != 999) {
                       real cumulative_conditional_prob = Phi(beta_nd[s] - C_non_diseased[s, 1]);
                       target += binomial_lpmf(  x_non_diseased[s, 1] | n_non_diseased[s], cumulative_conditional_prob );
              }
              
              for (k in 2:n_thr) {
                   if (x_non_diseased[s, k] != 999) {
                        real cumulative_conditional_prob = Phi(beta_nd[s] - C_non_diseased[s, k]) / Phi(beta_nd[s] - C_non_diseased[s, k - 1]);
                        target += binomial_lpmf(  x_non_diseased[s, k] | x_non_diseased[s, k - 1], cumulative_conditional_prob );
                   }
              }
              
              //// Diseased group (D+):
              if (x_diseased[s, 1] != 999) {
                       real cumulative_conditional_prob = Phi((beta_d[s] - C_diseased[s, 1])/scale_d[s]);
                       target += binomial_lpmf(  x_diseased[s, 1] | n_diseased[s], cumulative_conditional_prob );
              }
              
              for (k in 2:n_thr) {
                 if (x_diseased[s, k] != 999) {
                      real cumulative_conditional_prob = Phi((beta_d[s] - C_diseased[s, k])/scale_d[s]) /  Phi((beta_d[s] - C_diseased[s, k - 1])/scale_d[s]);
                      target += binomial_lpmf( x_diseased[s, k] | x_diseased[s, k - 1], cumulative_conditional_prob );
                 }
              }
