...
transformed parameters { 
        ...
        for (s in 1:n_studies) {
                for (c in 1:2) {
                      for (cut_i in 1:to_int(n_cutpoints[c, s])) {
                              ...
                              logit_cumul_prob[c][s, cut_i] = (beta[c, s] - C[k])/scale[c, s];
                      } 
                }
        }
        ...
        for (s in 1:n_studies) {
         ...
         //// Induced-Dirichlet ** prior model ** stuff:
         {
               ...
               Ind_Dir_ord_prob[1] = 1.0 - Ind_Dir_cumul_prob[1];
               for (k in 2:n_thr) {
                   Ind_Dir_ord_prob[k] = (Ind_Dir_cumul_prob[k - 1] - Ind_Dir_cumul_prob[k])/scale_mu; // since probs are decreasing with k
               }
               Ind_Dir_ord_prob[n_cat] =  Ind_Dir_cumul_prob[n_cat - 1];
         }
...
model {
        ...
        for (c in 1:2) {
               target += induced_dirichlet_v2_lpdf(Ind_Dir_ord_prob[1:n_cat] | Ind_Dir_cumul_prob[1:n_thr], prior_alpha);
        }
        //// Likelihood / Model:
        {
...
generated quantities {
          ...
          for (k in 1:n_thr) {
                Fp[k] =   inv_logit((beta_mu[1] - C[k])/scale_mu[1]);
                Sp[k] =   1.0 - Fp[k];
                Se[k] =   inv_logit((beta_mu[2] - C[k])/scale_mu[2]);
          }
          //// Calculate predictive accuracy:
          {
                ...
                for (k in 1:n_thr) {
                      Fp_pred[k] =   inv_logit((beta_pred[1] - C[k])/scale_pred[1]);
                      Sp_pred[k] =   1.0 - Fp_pred[k];
                      Se_pred[k] =   inv_logit((beta_pred[2] - C[k])/scale_pred[2]);
                }
          }
