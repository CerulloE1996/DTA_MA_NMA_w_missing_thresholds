        ...
        for (s in 1:n_studies) {
                  for (c in 1:2) {
                        for (cut_i in 1:to_int(n_cutpoints[c, s])) {
                                ...
                                logit_cumul_prob[c][s, cut_i] = (beta[c, s] - C[s, k]);
                        }
                  }
        }
...
generated quantities {
          ...
          for (k in 1:n_thr) {
                Fp[k] =   inv_logit((beta_mu[1] - C_mu[k]));
                Sp[k] =   1.0 - Fp[k];
                Se[k] =   inv_logit((beta_mu[2] - C_mu[k]));
          }
          ...
          //// Calculate predictive accuracy:
          {
                ...
                for (k in 1:n_thr) {
                      Fp_pred[k] =   inv_logit((beta_pred[1] - C_mu[k]));
                      Sp_pred[k] =   1.0 - Fp_pred[k];
                      Se_pred[k] =   inv_logit((beta_pred[2] - C_mu[k]));
                }
          }
