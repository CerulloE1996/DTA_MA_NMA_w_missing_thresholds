...
model {
      ...
      for (s in 1:n_studies)  {
                C_diseased[s, 1:n_thr]      ~  induced_dirichlet(alpha_d, 0.0);
                C_non_diseased[s, 1:n_thr]  ~  induced_dirichlet(alpha_nd, 0.0);
      }
      for (s in 1:n_studies) {
              
              //// Non-diseased group (fixed parameters)
              if (x_non_diseased[s, 1] != 999) {
                  x_non_diseased[s, 1] ~ binomial( n_non_diseased[s], Phi(C_non_diseased[s, 1]) );
              }
              
              for (k in 2:n_thr) {
                   if (x_non_diseased[s, k] != 999) {
                    x_non_diseased[s, k] ~ binomial( x_non_diseased[s, k-1], Phi(C_non_diseased[s, k]) / Phi(C_non_diseased[s, k-1]) );
                   }
              }
              
              //// Diseased group (D+):
              if (x_diseased[s, 1] != 999) {
                 x_diseased[s, 1] ~ binomial( n_diseased[s], Phi((beta_d[s] - C_diseased[s, 1])/scale_d[s]) );
              }
              
              for (k in 2:n_thr) {
                 if (x_diseased[s, k] != 999) {
                      x_diseased[s, k] ~ binomial( x_diseased[s, k-1], Phi((beta_d[s] - C_diseased[s, k])/scale_d[s]) /  Phi((beta_d[s] - C_diseased[s, k-1])/scale_d[s]) );
                 }
              }
...
generated quantities {
        ...
        for (s in 1:n_studies) {
              for (k in 1:n_thr) {
                  se[s][k] = 1.0 - Phi((C_diseased[s, k] - beta_d[s])/scale_d[s]);
                  sp[s][k] = Phi(C_non_diseased[s, k]);
              }
        }
        
        //// Calculate summary accuracy (using mean parameters):
        for (k in 1:n_thr) {
            Se[k] = 1.0 - Phi((C_diseased_mu[k] - beta_d_mu)/exp(log_scale_d_mu));
            Sp[k] = Phi(C_non_diseased_mu[k]);
        }
        ...
        //// Calculate predictive accuracy:
        {
            ...
            for (k in 1:n_thr) {
                Se_pred[k] = 1 - Phi((C_diseased_mu[k] - beta_d_pred)/scale_d_pred);
                Sp_pred[k] = Phi(C_non_diseased_mu[k]);
            }
        }
